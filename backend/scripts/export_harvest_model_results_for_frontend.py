...
def export_model_dashboard(
    *,
    ...
    reports = backend_root / "artifacts" / "reports" / "harvesting" / "reviewed" / "research_models"

    selected_metrics = _read_json(reports / "selected_model_metrics.json")
    comparison = _read_csv(reports / "model_feature_set_comparison.csv")
    validation_events = _read_csv(reports / "selected_validation_event_detection.csv")
    test_events = _read_csv(reports / "selected_test_event_detection.csv")
    robustness = _read_csv(reports / "selected_grouped_hive_robustness.csv")
    feature_importance = _read_csv(reports / "selected_feature_importance.csv")
    session_summary = _read_csv(reports / "harvest_session_summary.csv")
    ...
    if sort_columns:
        ascending = [column == "validation_false_alert_episodes" for column in sort_columns]
    ...
    selected_feature_set = selected_metrics["selected_feature_set"]

    comparison["selected"] = comparison["model"].eq(selected_model) & comparison["feature_set"].eq(
        selected_feature_set
    )

    output_root = frontend_root / "public" / "data" / "harvesting-models"
    ...
    payload = {
        ...
        "feature_importance": _records(feature_importance.head(25)),
        ...
        "display_notes": [
            ...
            ("Validation contains only two reviewed events and test contains one reviewed event."),
            ("The displayed scores are not yet calibrated HUI probabilities."),
            (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=("Export reviewed harvesting model-comparison results for the Vite dashboard.")
    )
    parser.add_argument(
        "--frontend-root",
        default=None,
