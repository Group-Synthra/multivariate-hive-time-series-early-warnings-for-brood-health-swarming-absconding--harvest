...
def evaluate_forecasting_research_gate(
    ...
    *,
    ...
    for horizon in horizons_hours:
        ...
        persistence_rows = horizon_rows.loc[
            horizon_rows["model"].eq("persistence")
        ]
        if persistence_rows.empty:
            raise ValueError(
                f"No successful persistence baseline exists at {horizon}h."
            )

        baseline_mae = float(
            persistence_rows["validation_mae"].min()
        )
        ...
        horizon_passed = bool(
            improvement_passed and generalization_passed
        )
    ...
    if require_72h_not_worse_than_persistence:
        if horizon_72 is None:
            raise ValueError(
                "The 72-hour forecasting horizon is required."
            )
    ...
    ready = bool(
        improved_horizon_count >= required_improved_horizons
        and horizon_72_passed
    )

    return {
        "status": (
            "forecasting_gate_passed"
            if ready
            else "forecasting_gate_failed"
        ),
        ...
        "require_72h_not_worse_than_persistence": (
            require_72h_not_worse_than_persistence
        ),
...
def assign_readiness_class(
    ...
    if not approaching <= ready <= high_priority:
        raise ValueError(
            "Readiness thresholds must be monotonically increasing."
        )
...
def assign_candidate_window(
    ...
    *,
    ...
    eligible = frame[class_column].isin(
        ["Ready", "High Priority"]
    )
    rate_24 = frame[
        "predicted_rate_24h_kg_per_hour"
    ...
    labels = np.select(
        [
            eligible
            & rate_24.le(
                float(plateau_rate_thresholds["24"])
            ),
            eligible
            & rate_48.le(
                float(plateau_rate_thresholds["48"])
            ),
            eligible
            & rate_72.le(
                float(plateau_rate_thresholds["72"])
            ),
        ],
        [
            "0-24 hours",
    ...
    for horizon in horizons_hours:
        ...
        if not model_path.exists():
            raise FileNotFoundError(
                f"Selected forecaster not found: {model_path}"
            )

        model_metadata = _read_json(metadata_path)
        feature_columns = [
            str(value)
            for value in model_metadata["feature_columns"]
        ]
        _require_columns(
            result,
            set(feature_columns),
        ...
        result[f"predicted_delta_{horizon}h_kg"] = (
...
def build_provisional_readiness_scores(
    ...
    *,
    ...
    if required_weights != REQUIRED_COMPONENTS:
        raise ValueError(
            "Component weights must contain exactly: "
    ...
    if not np.isclose(weight_total, 1.0):
        raise ValueError(
            "Readiness component weights must sum to one."
        )
    ...
    required_columns.update(
        {
            f"predicted_delta_{horizon}h_kg"
            for horizon in horizons_hours
        }
    )
    required_columns.update(
    ...
    if not train_mask.any():
        raise ValueError(
            "Training rows are required for readiness normalization."
        )

    recent_accumulation = frame[
        "weight_delta_72h_kg"
    ].clip(lower=0.0)
    distance_from_max = frame[
        "weight_distance_from_max_168h_kg"
    ].clip(lower=0.0)
    ...
    mean_absolute_future_rate = (
        predicted_rates.abs().mean(axis=1)
    )
    ...
    future_rate_72 = frame[
        "predicted_rate_72h_kg_per_hour"
    ].abs()
    forecast_slowdown = (
        recent_rate - future_rate_72
    ).clip(lower=0.0)
    environmental_variability = (
        frame["temperature_c_std_24h"]
        + frame["co2_ppm_std_24h"] / 100.0
    )
    ...
    frame["hrsi"] = (
        1.0
        - (
            rolling_std.fillna(stability_scale)
            / stability_scale
        ).clip(0.0, 1.0)
    ) * 100.0

    frame["hrroc_points_per_hour"] = (
        grouped.diff(periods=rate_of_change_hours)
        / rate_of_change_hours
    )
    ...
    frame["candidate_harvest_window"] = (
    ...
    parameters["stability_window_hours"] = (
        stability_window_hours
    )
    parameters["rate_of_change_hours"] = (
        rate_of_change_hours
    )
    parameters["plateau_rate_thresholds"] = (
        plateau_thresholds
    )
...
def run_provisional_readiness_from_config(
    *,
    ...
    if not bool(
        gate.get("ready_for_readiness_prototype", False)
    ):
        raise RuntimeError(
    ...
    enriched, forecaster_metadata = (
        _load_forecaster_predictions(
            feature_rows,
    ...
    readiness, parameters, thresholds = (
        build_provisional_readiness_scores(
            ...
            plateau_rate_quantile=float(
                normalization["plateau_rate_quantile"]
            ),
            stability_window_hours=int(
                settings["stability_window_hours"]
            ),
            stability_minimum_periods=int(
                settings["stability_minimum_periods"]
            ),
            rate_of_change_hours=int(
                settings["rate_of_change_hours"]
            ),
            class_quantiles={
                str(key): float(value)
                for key, value in settings[
                    "readiness_class_quantiles"
                ].items()
            },
        )
    )
