...
def create_event_aware_folds(
    ...
    *,
    ...
    if missing_columns:
        raise ValueError(f"Missing columns required for event-aware splitting: {missing_columns}")

    if minimum_training_events <= 0:
        raise ValueError("minimum_training_events must be greater than zero")

    if validation_events_per_fold <= 0:
        raise ValueError("validation_events_per_fold must be greater than zero")
    ...
    training_events = events.loc[events[split_column].astype("string").eq("train")].copy()

    if "is_boundary_gap" in training_events.columns:
        training_events = training_events.loc[
            ~training_events["is_boundary_gap"].fillna(False).astype(bool)
        ].copy()
    ...
    training_events = training_events.sort_values(event_start_column).reset_index(drop=True)

    required_event_count = minimum_training_events + validation_events_per_fold
    ...
    prediction_horizon = pd.Timedelta(hours=prediction_horizon_hours)
    ...
    while validation_begin_index < len(training_events):
        validation_end_index = min(
            validation_begin_index + validation_events_per_fold,
            len(training_events),
        )

        validation_group = training_events.iloc[validation_begin_index:validation_end_index]

        if validation_group.empty:
            break

        first_validation_event = validation_group[event_start_column].min()

        last_validation_event = validation_group[event_start_column].max()

        # Include the complete future-prediction period before the
        # first validation event.
        validation_start = first_validation_event - prediction_horizon

        # Training ends before the purge interval. This prevents
        # a training target from looking forward into validation.
        train_end = validation_start - purge_interval

        eligible_training_events = training_events.loc[
            training_events[event_start_column] <= train_end
        ]

        if len(eligible_training_events) >= minimum_training_events:
            folds.append(
                HarvestFold(
                    fold=fold_number,
                    train_start=eligible_training_events[event_start_column].min(),
                    train_end=train_end,
                    validation_start=validation_start,
                    validation_end=last_validation_event,
                    training_events=len(eligible_training_events),
                    validation_events=len(validation_group),
                )
            )
