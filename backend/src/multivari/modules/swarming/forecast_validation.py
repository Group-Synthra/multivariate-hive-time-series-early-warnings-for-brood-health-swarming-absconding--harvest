...
DATA_FILE = Path(
    os.getenv(
        ...
        / "data"
        / "swarming_forecast_validation.json",
    )
)
...
def _iso(value: datetime) -> str:
    """Convert a datetime value to an ISO UTC timestamp."""
    return value.astimezone(UTC).isoformat().replace("+00:00", "Z")


def _load_rows() -> list[dict[str, Any]]:
    """Load forecast-validation records from the JSON file."""
...
def _save_rows(rows: list[dict[str, Any]]) -> None:
    """Save forecast-validation records using an atomic file replacement."""
...
def _risk_level(value: float) -> str:
    """Convert a risk percentage into its corresponding risk level."""
    if value >= 60:
        return "HIGH"
...
def _round_risk(value: Any) -> float:
    """Validate and round a risk percentage."""
    risk = float(value)
...
def _normalise_forecasts(forecasts: Any) -> list[dict[str, Any]]:
    """Validate and normalise the three forecast horizons."""
    ...
    for item in forecasts:
        if not isinstance(item, dict):
            raise TypeError("Each forecast must be an object")
        ...
        normalised.append(
            {
                "day": day,
                "risk": _round_risk(item.get("risk")),
            }
        )
...
def _migrate_old_day1_rows(rows: list[dict[str, Any]]) -> None:
    """Keep records created by the earlier Day-1-only version usable."""
    for row in rows:
...
def _validate_pending_rows(
    ...
    *,
    ...
    """Validate the closest pending row for every forecast horizon."""
    for forecast_day in sorted(VALID_FORECAST_DAYS):
        ...
        for row in rows:
            ...
            difference_seconds = abs(
                (
                    observation_time - _parse_timestamp(row["target_at"])
                ).total_seconds()
            )
        ...
        difference_seconds, matched = min(
            candidates,
        ...
        matched.update(
            {
                ...
                "level_match": matched["forecast_level"] == actual_level,
                "status": "VALIDATED",
...
def record_and_match(
    *,
    ...
    """Validate previous forecasts and record the latest forecasts."""
    ...
    with _FILE_LOCK:
        ...
        # Validate forecasts made one, two, or three days earlier.
        ...
        # Store today's Day-1, Day-2, and Day-3 forecasts separately.
...
def save_forecast_validation():
    """Save new forecasts and validate matching previous forecasts."""
...
def get_forecast_validation():
    """Return stored forecast-validation records."""
