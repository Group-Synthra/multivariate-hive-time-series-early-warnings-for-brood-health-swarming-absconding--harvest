"""
=========================================================
Risk Level Classifier with Softmax Probability
=========================================================

Implements:
    P(swarming | x) =
    e^(z_swarm) / (e^(z_swarm) + e^(z_non-swarm))

Outputs:
    - Risk Percentage (0-100%)
    - Predicted Event Window (24-72 hours)
    - Risk Level (High/Medium/Low)
    - Key Contributing Factors
=========================================================
"""

from __future__ import annotations

import logging
from datetime import UTC, datetime

import numpy as np

logger = logging.getLogger(__name__)


class RiskClassifier:
    """Classify swarming risk and provide recommendations."""

    def __init__(self) -> None:
        """Initialize the classifier with risk thresholds."""
        self.RISK_LOW_MAX = 30
        self.RISK_MEDIUM_MAX = 60

        self.levels = ["LOW", "MEDIUM", "HIGH"]

        self.WINDOW_MAP = {
            "LOW": {
                "window": "No immediate risk predicted",
                "hours": None,
                "confidence": "High",
                "urgency": "Routine",
            },
            "MEDIUM": {
                "window": "Potential swarming within 48-72 hours",
                "hours": 60,
                "confidence": "Medium",
                "urgency": "Soon",
            },
            "HIGH": {
                "window": "Swarming likely within 24-48 hours",
                "hours": 36,
                "confidence": "High",
                "urgency": "Immediate",
            },
        }

        self.RISK_MESSAGES = {
            "LOW": ("Hive behaviour is normal. No immediate swarming risk detected."),
            "MEDIUM": ("Possible behavioural changes detected. Continue monitoring hive."),
            "HIGH": ("High swarming probability detected. Immediate hive inspection recommended."),
        }

        self.feature_names = [
            "internal_temperature_c",
            "internal_humidity_pct",
            "co2_ppm",
            "hive_weight_kg",
            "external_temperature_c",
            "external_humidity_pct",
            "rainfall_mm_hour",
            "wind_speed_mps",
            "breakpoint",
            "days_since_breakpoint",
            "breakpoint_density",
            "segment_duration",
        ]

        self.normal_ranges = {
            "internal_temperature_c": (30, 36),
            "internal_humidity_pct": (50, 80),
            "co2_ppm": (400, 1500),
            "hive_weight_kg": (20, 50),
            "external_temperature_c": (15, 35),
            "external_humidity_pct": (40, 80),
            "battery_voltage": (3.0, 4.5),
            "breakpoint_density": (0, 1),
        }

    def softmax_binary(self, logits: float) -> float:
        """
        Calculate binary Softmax swarming probability.

        P(swarming | x) =
        e^(z_swarm) / (e^(z_swarm) + e^(z_non-swarm))
        """
        z_swarm = logits
        z_non_swarm = 0.0

        e_swarm = np.exp(z_swarm)
        e_non_swarm = np.exp(z_non_swarm)

        probability = e_swarm / (e_swarm + e_non_swarm)

        return float(np.clip(probability, 0.0001, 0.9999))

    def sigmoid_to_logits(self, sigmoid_output: float) -> float:
        """Convert a sigmoid output into a logit value."""
        probability = np.clip(sigmoid_output, 0.0001, 0.9999)
        return float(np.log(probability / (1 - probability)))

    def calculate_pelt_risk(
        self,
        pelt_snapshot: dict | None,
    ) -> dict:
        """Convert the latest PELT state into a 0-100 risk score."""
        snapshot = pelt_snapshot or {}

        breakpoint = int(snapshot.get("breakpoint", 0) or 0)
        density = max(
            0.0,
            float(snapshot.get("breakpoint_density", 0) or 0),
        )
        steps_since = max(
            0.0,
            float(snapshot.get("days_since_breakpoint", 0) or 0),
        )

        density_risk = min(100.0, density * 15.0)

        recency_risk = max(0.0, 40.0 - steps_since * 1.5) if density > 0 else 0.0

        breakpoint_bonus = 15.0 if breakpoint == 1 else 0.0

        pelt_risk = np.clip(
            0.50 * density_risk + 0.30 * recency_risk + 0.20 * breakpoint_bonus,
            0.0,
            100.0,
        )

        return {
            "pelt_risk_percentage": round(float(pelt_risk), 2),
            "density_risk": round(float(density_risk), 2),
            "recency_risk": round(float(recency_risk), 2),
            "breakpoint_bonus": round(float(breakpoint_bonus), 2),
        }

    def combine_lstm_and_pelt(
        self,
        lstm_probability: float,
        pelt_snapshot: dict | None,
        lstm_weight: float = 0.70,
        pelt_weight: float = 0.30,
    ) -> dict:
        """Combine the LSTM probability and explicit PELT risk score."""
        if not np.isclose(lstm_weight + pelt_weight, 1.0):
            raise ValueError("LSTM and PELT weights must add up to 1.0.")

        lstm_risk = float(np.clip(lstm_probability, 0.0, 1.0) * 100.0)

        pelt_details = self.calculate_pelt_risk(pelt_snapshot)
        pelt_risk = pelt_details["pelt_risk_percentage"]

        combined_risk = float(
            np.clip(
                lstm_weight * lstm_risk + pelt_weight * pelt_risk,
                0.0,
                100.0,
            )
        )

        return {
            "lstm_risk_percentage": round(lstm_risk, 2),
            **pelt_details,
            "risk_percentage": round(combined_risk, 2),
            "combined_probability": round(
                combined_risk / 100.0,
                6,
            ),
            "lstm_weight": lstm_weight,
            "pelt_weight": pelt_weight,
            "formula": ("0.70 × LSTM risk + 0.30 × explicit PELT risk"),
        }

    def classify_from_probability(
        self,
        probability: float,
    ) -> dict:
        """Classify risk from the LSTM sigmoid probability."""
        if 0 <= probability <= 1:
            logits = self.sigmoid_to_logits(probability)
        else:
            logits = probability

        risk_probability = self.softmax_binary(logits)
        risk_percentage = risk_probability * 100

        risk_level = self._get_risk_level(risk_percentage)
        event_window = self._get_event_window(
            risk_level,
            risk_percentage,
        )
        message = self.RISK_MESSAGES[risk_level]

        softmax_probabilities = self._get_softmax_probabilities(risk_percentage)

        return {
            "risk_percentage": round(risk_percentage, 2),
            "risk_level": risk_level,
            "probability": round(risk_probability, 4),
            "event_window": event_window,
            "message": message,
            "softmax_probabilities": softmax_probabilities,
            "formula": ("P(swarming | x) = e^(z_swarm) / (e^(z_swarm) + e^(z_non-swarm))"),
            "timestamp": datetime.now(UTC).isoformat(),
        }

    def classify_with_factors(
        self,
        probability: float,
        sequence: np.ndarray | None = None,
        sensor_values: dict | None = None,
    ) -> dict:
        """Classify risk and identify important sensor factors."""
        result = self.classify_from_probability(probability)

        if sensor_values:
            result["key_factors"] = self._identify_key_factors(sensor_values)

        result["recommendations"] = self._get_recommendations(
            result["risk_level"],
            result.get("key_factors", []),
        )

        return result

    def _get_risk_level(
        self,
        risk_percentage: float,
    ) -> str:
        """Map a risk percentage to LOW, MEDIUM, or HIGH."""
        if risk_percentage <= self.RISK_LOW_MAX:
            return "LOW"

        if risk_percentage <= self.RISK_MEDIUM_MAX:
            return "MEDIUM"

        return "HIGH"

    def _get_event_window(
        self,
        risk_level: str,
        risk_percentage: float,
    ) -> dict:
        """Estimate the possible swarming event window."""
        base = self.WINDOW_MAP[risk_level].copy()

        if risk_level == "MEDIUM":
            hours = int(72 - (risk_percentage - 30) * 1.2)
            hours = max(48, min(72, hours))

            base["window"] = f"Potential swarming within {hours - 12} to {hours + 12} hours"
            base["hours"] = hours
            base["range"] = [
                hours - 12,
                hours + 12,
            ]

        elif risk_level == "HIGH":
            hours = int(48 - (risk_percentage - 60) * 0.8)
            hours = max(24, min(48, hours))

            base["window"] = f"Swarming likely within {hours - 6} to {hours + 6} hours"
            base["hours"] = hours
            base["range"] = [
                hours - 6,
                hours + 6,
            ]

        return base

    def _get_softmax_probabilities(
        self,
        risk_percentage: float,
    ) -> dict:
        """Create a probability distribution across risk levels."""
        normalized_risk = risk_percentage / 100

        low_score = max(
            0.01,
            1 - normalized_risk * 1.5,
        )
        medium_score = max(
            0.01,
            1 - abs(normalized_risk - 0.5) * 2,
        )
        high_score = max(
            0.01,
            normalized_risk * 1.5,
        )

        scores = np.array(
            [
                low_score,
                medium_score,
                high_score,
            ]
        )

        exponential_scores = np.exp(scores - np.max(scores))
        probabilities = exponential_scores / exponential_scores.sum()

        return {
            "LOW": round(probabilities[0] * 100, 2),
            "MEDIUM": round(probabilities[1] * 100, 2),
            "HIGH": round(probabilities[2] * 100, 2),
        }

    def get_risk_color(
        self,
        risk_level: str,
    ) -> str:
        """Return the display color for a risk level."""
        colors = {
            "LOW": "#22c55e",
            "MEDIUM": "#eab308",
            "HIGH": "#ef4444",
        }

        return colors.get(risk_level, "#94a3b8")

    def _identify_key_factors(
        self,
        sensor_values: dict,
    ) -> list[dict]:
        """Identify sensors outside their normal ranges."""
        factors = []

        for key, value in sensor_values.items():
            if key not in self.normal_ranges:
                continue

            if value is None:
                continue

            low, high = self.normal_ranges[key]

            try:
                numeric_value = float(value)
            except (ValueError, TypeError):
                continue

            if numeric_value < low:
                deviation = ((low - numeric_value) / low) * 100

                if deviation > 5:
                    influence = self._get_influence(deviation)

                    factors.append(
                        {
                            "feature": key,
                            "value": round(
                                numeric_value,
                                2,
                            ),
                            "deviation": (f"{deviation:.1f}% below normal"),
                            "influence": influence,
                        }
                    )

            elif numeric_value > high:
                deviation = ((numeric_value - high) / high) * 100

                if deviation > 5:
                    influence = self._get_influence(deviation)

                    factors.append(
                        {
                            "feature": key,
                            "value": round(
                                numeric_value,
                                2,
                            ),
                            "deviation": (f"{deviation:.1f}% above normal"),
                            "influence": influence,
                        }
                    )

        influence_order = {
            "High": 0,
            "Medium": 1,
            "Low": 2,
        }

        factors.sort(
            key=lambda factor: influence_order.get(
                factor["influence"],
                3,
            )
        )

        return factors[:5]

    @staticmethod
    def _get_influence(
        deviation: float,
    ) -> str:
        """Map a percentage deviation to an influence level."""
        if deviation < 20:
            return "Low"

        if deviation < 50:
            return "Medium"

        return "High"

    def _get_recommendations(
        self,
        risk_level: str,
        factors: list[dict],
    ) -> list[str]:
        """Return recommendations based on risk and factors."""
        recommendations = {
            "LOW": [
                "✓ Continue routine hive monitoring",
                "✓ Maintain normal hive ventilation",
                "✓ Ensure adequate food and water availability",
                "✓ Review sensor readings during the next update",
            ],
            "MEDIUM": [
                "⚠️ Conduct hive inspection within 24-48 hours",
                "⚠️ Check for the presence of queen cells",
                "⚠️ Monitor colony population and congestion",
                "⚠️ Consider providing additional space if needed",
                "⚠️ Review sensor data daily",
            ],
            "HIGH": [
                "🚨 Perform immediate hive inspection TODAY!",
                "🚨 Check ALL frames for queen cells",
                "🚨 Reduce overcrowding or consider hive splitting",
                "🚨 Prepare swarm traps if possible",
                "🚨 Monitor temperature and CO₂ levels closely",
                "🚨 Contact an experienced beekeeper if needed",
            ],
        }

        base_recommendations = recommendations.get(
            risk_level,
            recommendations["LOW"],
        )

        factor_recommendations = []

        for factor in factors[:2]:
            feature = factor["feature"]
            deviation = factor["deviation"]

            if "temperature" in feature.lower():
                factor_recommendations.append(f"  → Temperature is {deviation} - check ventilation")

            elif "weight" in feature.lower():
                factor_recommendations.append(f"  → Hive weight is {deviation} - check food stores")

            elif "co2" in feature.lower():
                factor_recommendations.append(f"  → CO₂ is {deviation} - check hive congestion")

            elif "humidity" in feature.lower():
                factor_recommendations.append(
                    f"  → Humidity is {deviation} - check moisture levels"
                )

        return base_recommendations + factor_recommendations

    def format_report(
        self,
        classification: dict,
    ) -> str:
        """Format a classification result as a readable report."""
        lines = [
            "=" * 70,
            "SWARMING PREDICTION REPORT",
            "=" * 70,
            "",
            (f"📊 Risk Percentage: {classification['risk_percentage']}%"),
            (f"📈 Risk Level: {classification['risk_level']}"),
        ]

        if classification.get("event_window"):
            event_window = classification["event_window"]

            lines.append(f"🎯 Event Window: {event_window['window']}")

        lines.append("")

        if classification.get("softmax_probabilities"):
            probabilities = classification["softmax_probabilities"]

            lines.extend(
                [
                    "📊 Probability Distribution:",
                    f"  • LOW: {probabilities['LOW']}%",
                    f"  • MEDIUM: {probabilities['MEDIUM']}%",
                    f"  • HIGH: {probabilities['HIGH']}%",
                    "",
                ]
            )

        if classification.get("key_factors"):
            lines.append("🔑 Key Factors:")

            for factor in classification["key_factors"]:
                lines.append(f"  • {factor['feature']}: {factor['value']} ({factor['deviation']})")

            lines.append("")

        lines.append("📝 Recommendations:")

        if classification.get("recommendations"):
            for recommendation in classification["recommendations"]:
                lines.append(f"  {recommendation}")
        else:
            lines.append(
                "  "
                + classification.get(
                    "message",
                    "No recommendations available",
                )
            )

        formula = classification.get(
            "formula",
            ("P(swarming | x) = e^(z_swarm) / (e^(z_swarm) + e^(z_non-swarm))"),
        )

        lines.extend(
            [
                "",
                "📐 Formula Used:",
                f"  {formula}",
                "",
                "=" * 70,
            ]
        )

        return "\n".join(lines)


def create_risk_classifier() -> RiskClassifier:
    """Create and return a RiskClassifier instance."""
    return RiskClassifier()


if __name__ == "__main__":
    classifier = RiskClassifier()

    print("Risk Colors:")
    print(f"  LOW: {classifier.get_risk_color('LOW')}")
    print(f"  MEDIUM: {classifier.get_risk_color('MEDIUM')}")
    print(f"  HIGH: {classifier.get_risk_color('HIGH')}")

    for probability in [
        0.02,
        0.15,
        0.45,
        0.85,
    ]:
        result = classifier.classify_from_probability(probability)

        print(
            f"\nProbability: {probability:.2f} → "
            f"{result['risk_level']} "
            f"({result['risk_percentage']:.1f}%)"
        )
        print(f"  Event: {result['event_window']['window']}")
        print(f"  Distribution: {result['softmax_probabilities']}")
