...
def save_risk_prediction(
    ...
    """Save or update a risk for one real IoT data timestamp."""
    predicted_at = (
    ...
    record = {
        ...
        "pelt_risk_percentage": float(prediction.get("pelt_risk_percentage", 0)),
        ...
        "predicted_class": prediction.get("predicted_class", "No Swarming"),
    }
    ...
    with _file_lock:
        ...
        history = [
            ...
            for item in history
            if (
                (timestamp := _parse_timestamp(str(item.get("predicted_at", "")))) is not None
        ...
        history.sort(key=lambda item: item.get("predicted_at", ""))
...
def get_risk_history(
    ...
    filtered_history = []
    for item in history:
        timestamp = _parse_timestamp(str(item.get("predicted_at", "")))
        if timestamp is not None and timestamp >= cutoff and item.get("device_id") == device_id:
            filtered_history.append(item)

    filtered_history.sort(key=lambda item: item.get("predicted_at", ""))
