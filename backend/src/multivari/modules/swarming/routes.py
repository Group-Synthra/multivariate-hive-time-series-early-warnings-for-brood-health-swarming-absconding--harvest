...
def _generate_sample_readings(n: int = 144) -> list:
    ...
    for i in range(n):
        ...
        readings.append({
            "recorded_at": (
                datetime.now(timezone.utc) - timedelta(minutes=10 * (n - 1 - i))
            ).isoformat(),
...
def sample_payload():
    ...
    sample = {
        "hive_id" : "Hive_01",
        "readings": _generate_sample_readings(144)
    }
    ...
    try:
        ...
        def _get_sensor_trend(readings):
            ...
            if "recorded_at" not in sensor_df or "internal_temperature_c" not in sensor_df:
                return 0
            sensor_df["recorded_at"] = pd.to_datetime(sensor_df["recorded_at"], utc=True, errors="coerce")
            sensor_df["internal_temperature_c"] = pd.to_numeric(sensor_df["internal_temperature_c"], errors="coerce")
            hourly_temp = (
                sensor_df.dropna(subset=["recorded_at"])
                .set_index("recorded_at")["internal_temperature_c"]
                .resample("1h").mean().interpolate().dropna().tail(24)
            )
