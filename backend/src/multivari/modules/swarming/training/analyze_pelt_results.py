...
if SWARMING_EVENT_COLUMN in df.columns:
    ...
    print(f"   Total rows with label=1: {df[SWARMING_EVENT_COLUMN].sum():,}")
    print(
        f"   Average hours per event: {df[SWARMING_EVENT_COLUMN].sum() / total_events if total_events > 0 else 0:.1f}"
    )

    events_per_hive = (
        df[df[SWARMING_EVENT_COLUMN] == 1].groupby(HIVE_COLUMN)["date"].nunique().reset_index()
    )
    events_per_hive.columns = [HIVE_COLUMN, "swarming_events"]
    ...
    events_per_hive = (
        df[df[SWARMING_LABEL] == 1].groupby(HIVE_COLUMN)["date"].nunique().reset_index()
    )
    events_per_hive.columns = [HIVE_COLUMN, "swarming_events"]
...
breakpoint_summary["breakpoints_per_100h"] = (
    breakpoint_summary["total_breakpoints"] / breakpoint_summary["total_records"] * 100
)

# Merge with swarming events (unique counts)
if len(events_per_hive) > 0:
    breakpoint_summary = breakpoint_summary.merge(events_per_hive, on=HIVE_COLUMN, how="left")
    breakpoint_summary["swarming_events"] = (
        breakpoint_summary["swarming_events"].fillna(0).astype(int)
    )
else:
    breakpoint_summary["swarming_events"] = 0

# Sort by breakpoints descending
breakpoint_summary = breakpoint_summary.sort_values("total_breakpoints", ascending=False)
...
ax.set_title(
    f"Table 1: All {num_hives} Hives with Breakpoints and Swarming Events",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
...
breakpoint_summary_sorted = breakpoint_summary.sort_values("total_breakpoints", ascending=True)
...
ax.set_ylabel("Hive ID", fontsize=12)
ax.set_title(
    f"All {len(hives)} Hives: Breakpoints with Swarming Events", fontsize=14, fontweight="bold"
)
ax.grid(axis="x", alpha=0.3)
...
ax.set_title(
    "Table 2: Overall PELT Statistics with Swarming Events", fontsize=14, fontweight="bold", pad=20
)
...
if len(hives_no_swarming) > 0:
    ax.scatter(
        hives_no_swarming["total_breakpoints"],
        hives_no_swarming["swarming_events"]
        + np.random.uniform(-0.02, 0.02, len(hives_no_swarming)),
        c="lightgray",
        s=50,
        alpha=0.5,
        label="No Swarming",
...
if len(hives_with_swarming) > 0:
    correlation = breakpoint_summary["total_breakpoints"].corr(
        breakpoint_summary["swarming_events"]
    )
    ax.text(
        0.95,
...
if "alignment_count" in df.columns:
    ...
    if "alignment_ratio" in df.columns:
        ...
        ax2.barh(
            hive_alignment[HIVE_COLUMN].head(15),
...
hives_with_swarms = breakpoint_summary[breakpoint_summary["swarming_events"] > 0][
    HIVE_COLUMN
].unique()
...
if len(hives_with_swarms) > 0:
    ...
    for hive_to_plot in hives_to_plot:
        ...
        ax.set_title(
            f"Breakpoints Over Time - Hive: {hive_to_plot}", fontsize=16, fontweight="bold"
        )
        ...
        for idx, (col, (color, label)) in enumerate(var_colors.items()):
            ...
            if col in hive_data.columns:
                bp_times = hive_data[hive_data[col] == 1][TIMESTAMP_COLUMN].values
                ax.scatter(
                    bp_times, [1] * len(bp_times), color=color, s=50, marker="|", linewidth=2
                )

                if "swarming_event_label" in hive_data.columns:
                    swarm_times = hive_data[hive_data["swarming_event_label"] == 1][
                        TIMESTAMP_COLUMN
                    ].values
                    ax.scatter(
                        swarm_times,
        ...
        fig.suptitle(
            f"Per-Variable Breakpoints and Swarming Events - Hive: {hive_to_plot}",
            fontsize=16,
            fontweight="bold",
            y=0.98,
        )
...
all_hives_sorted = breakpoint_summary.sort_values("swarming_events", ascending=False)[
    HIVE_COLUMN
].values
...
for hive in all_hives_sorted:
    ...
    if "swarming_event_label" in hive_data.columns:
        ...
        if len(swarm_times) > 0:
            swarm_dates = {pd.to_datetime(t).date() for t in swarm_times}
            swarm_count = len(swarm_dates)

    aligned_6h = check_alignment_window(
        co2_times, temp_times, hum_times, weight_times, window_hours=6
    )
    aligned_12h = check_alignment_window(
        co2_times, temp_times, hum_times, weight_times, window_hours=12
    )
    aligned_24h = check_alignment_window(
        co2_times, temp_times, hum_times, weight_times, window_hours=24
    )
    aligned_48h = check_alignment_window(
        co2_times, temp_times, hum_times, weight_times, window_hours=48
    )
...
ax.set_title(
    f"Table 3: Breakpoint Alignment Summary - ALL {num_hives_all} Hives (Multiple Time Windows)",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
...
print(
    f"   Total Hives with at least 1 alignment (24h): {(alignment_df_all['Aligned 24h'] > 0).sum()}"
)
...
if len(top_aligned_hives) > 0:
    print(
        f"   Finding aligned dates for top {len(top_aligned_hives)} hives: {list(top_aligned_hives)}"
    )
    ...
    for hive in top_aligned_hives:
        ...
        for item in dates:
            aligned_dates_data.append(
                {"Hive": hive, "Date": item["date"], "Window": item["window"]}
            )
    ...
    if len(aligned_dates_data) > 0:
        ...
        ax.set_title(
            "Table 4: Aligned Breakpoint Dates (Top Hives, 24-hour window)",
            fontsize=14,
            fontweight="bold",
            pad=20,
        )
...
print(
    " 11. breakpoint_alignment_24h_all_hives.png - Breakpoint alignment for ALL 48 hives (6h/12h/24h/48h windows)"
)
print(" 12. aligned_breakpoint_dates.png - Aligned breakpoint dates (NEW)")

print("\nCSV Files:")
print("  - pelt_summary_with_swarming.csv")
print("  - breakpoint_alignment_24h_all_hives.csv")
print("  - alignment_statistics_all_hives.csv")
print("  - aligned_breakpoint_dates.csv (NEW)")
